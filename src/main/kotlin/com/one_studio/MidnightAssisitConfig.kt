...
import java.io.File
object MidnightAssisitConfig { private val gson: Gson = GsonBuilder().setPrettyPrinting().create()
...
fun getTranslationKey(): String {return "option.midnight-assist.target_priority." + this.name.lowercase()}
}
enum class Preset {
NONE, PASSIVE_ONLY, HOSTILE_ONLY, PASSIVE_AND_HOSTILE, NEUTRAL_ONLY, NEUTRAL_AND_HOSTILE, NEUTRAL_AND_PASSIVE, ALL_MOBS, ALL_ENTITIES, OVERWORLD, NETHER, END;
fun getTranslationKey(): String {return "option.midnight-assist.preset." + this.name.lowercase()}
}
...
val isPassive = group == SpawnGroup.CREATURE || group == SpawnGroup.AMBIENT || group == SpawnGroup.WATER_CREATURE || group == SpawnGroup.WATER_AMBIENT || group == SpawnGroup.AXOLOTLS || group == SpawnGroup.UNDERGROUND_WATER_CREATURE
